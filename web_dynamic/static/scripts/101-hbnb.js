$(function () {
  ...
  $(document).on('click', 'span#review_span', function() {
    ...
    $.get(`http://0.0.0.0:5001/api/v1/places/${place_id}/reviews`, function(reviews, txt) {
        reviews.forEach(review => {
        
            console.log('review: ', review);
            let userId = review.user_id;
            $.get(`http://0.0.0.0:5001/api/v1/users/${userId}`, function(user, txt) {
                const html = `<li style="margin-top:0%"> 
                <h3>From ${user.first_name} ${user.last_name} the ${review.updated_at}”</h3> 
      ...
      success: function (data) {
        ...
        data.forEach(place => {
          const html = `<article>
                ...
                <div class="reviews">
					<h2>Reviews<span id='review_span' data-id=${place.id}>show</span></h2> 

					<ul id='review_list' style="margin-top:0%">
					</ul>
                </div> 
            </article>
                    `;
